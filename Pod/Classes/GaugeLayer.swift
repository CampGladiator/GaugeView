...
class GaugeLayer: CALayer {
    ...
    override func draw(in ctx: CGContext) {
        ...
        ctx.addArc(center: CGPoint(x: frame.size.width / 2, y: frame.size.height / 2), radius: radius - (thickness / 2), startAngle: 0, endAngle: CGFloat(2.0 * Float.pi), clockwise: false)
    ...
    private func convertDegreesToRadius(degrees: CGFloat) -> CGFloat {
        return ((CGFloat(Float.pi) * degrees) / 180.0)
    }
}
